...
contract BotEntrypoint is AccessControl, Safe {
    ...
    address public wETH;
    uint256 public inversionQuantity = 0.01 ether;

    bytes32 public BOT_ROLE = keccak256("BOT_ROLE");


    event TokensBought(address token, uint256 wETHQuantityExpended, uint256 tokenQuantityObtained);
    event TokensSold(address token, uint256 tokenQuantityExpended, uint256 wETHQuantityObtained);
    event WethRedeemed(uint256 quantity);

    modifier onlyBotOrMe() {
        require(hasRole(BOT_ROLE, msg.sender) || msg.sender == address(this), "BotEntrypoint: ONLY BOT OR ME");
        _;
    }
    ...
    function buyEntryPointBot(address token) external onlyRole(BOT_ROLE) {
        ...
        require(IERC20(token).balanceOf(address(this)) == 0, "BotEntrypoint: ALREADY HAVE TOKENS"); 

        address[] memory path = new address[](2);
        path[0] = wETH;
        path[1] = token;

        IUniswapV2Router02(dexRouter).swapExactTokensForTokens(inversionQuantity, 0, path, address(this), block.timestamp + 120);
        ...
        require(compareStrings(revertMessage, successRevertMessage), "BotEntrypoint: FAILING TRYING TO SELL");

        emit TokensBought(token, inversionQuantity, IERC20(token).balanceOf(address(this)));
    ...
    function sellEntryPointBot(address token) public onlyBotOrMe  {
        ...
        IUniswapV2Router02(dexRouter).swapExactTokensForTokens(IERC20(token).balanceOf(address(this)), 0, path, address(this), block.timestamp + 120);
    }
    ...
    function setInversionQuantity(uint256 _inversionQuantity) external onlyRole(DEFAULT_ADMIN_ROLE) {
        inversionQuantity = _inversionQuantity;
    }
    ...
    function withdrawFundsERC20(address target, address tokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getRevertMsg(
        ...
        if (returnData.length < 68)
            return "SUBSCRIBED_CONTRACT: REVERT_SILENTLY";
        assembly {
            // Slice the sighash.
    ...
    function shouldShell(address token) external view returns (bool) {
        ...
        uint[] memory amounts = IUniswapV2Router02(dexRouter).getAmountsOut(amountIn, path);
